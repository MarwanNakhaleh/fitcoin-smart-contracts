...
describe("Challenge Upgradeability", function () {
  ...
  beforeEach(async () => {
    ...
    const ChallengeFactory = await ethers.getContractFactory("Challenge", deployer);
    ...
    const vaultContract = await upgrades.deployProxy(VaultFactory, [challengeContractAddress], { initializer: 'initialize' });
  ...
  it("preserves all critical state during an upgrade with an active challenge", async () => {
    ...
    const challengerBetBefore = await challengeContact.challengeToBetsFor(challengeId, userAddress);
    const deployerBetBefore = await challengeContact.challengeToBetsAgainst(challengeId, deployerAddress);
    ...
    const challengerBetAfter = await challengeV2.challengeToBetsFor(challengeId, userAddress);
    const deployerBetAfter = await challengeV2.challengeToBetsAgainst(challengeId, deployerAddress);
